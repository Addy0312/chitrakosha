    ...
    <div className="container mx-auto p-4">
      ...
      {isArtist && (
        <>
          ...
          <CardContent>
            <AuctionCreationForm artworks={artworks} />
          </CardContent>
          </Card>
...
function AuctionCreationForm({ artworks }: { artworks: Artwork[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEndTime('');
      setStartingPrice('');
      window.location.reload(); // reload to update artwork status
    } catch (err: any) {
...
// AuctionCreationForm component (must be outside DashboardPage)
function AuctionCreationForm({ artworks }) {
...
// AuctionCreationForm component (move outside DashboardPage)
function AuctionCreationForm({ artworks }: { artworks: Artwork[] }) {
  const [selectedArtwork, setSelectedArtwork] = useState('');
  const [startTime, setStartTime] = useState('');
  const [endTime, setEndTime] = useState('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // For now, just show a toast/alert with the entered data
    alert(
      `Auction Created (Demo):\nArtwork: ${selectedArtwork}\nStart: ${startTime}\nEnd: ${endTime}\nStarting Price: ₹${startingPrice}`
    );
  };
